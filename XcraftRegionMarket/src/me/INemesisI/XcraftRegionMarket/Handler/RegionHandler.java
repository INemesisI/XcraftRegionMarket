...
public class RegionHandler {
	...
	public boolean canBuy(String player, String world, String type, ProtectedRegion region, Map<String, Integer> count) {
		...
		if (region.getParent() != null) {
			...
			if (limit.containsKey(key) && count.containsKey(key))
				if (limit.get(key) != -1 && limit.get(key) <= count.get(key)) {
		...
		if (type.equals("sell") || type.equals("sold")) {	
			...
			if (sellgroup != null) {
				for (String g : plugin.configHandler.getIgnoredgroups()) {
					if (permission.playerInGroup((String) null, player, g)) {
			...
			/*
			String[]  groups = permission.getPlayerGroups((String) null, player);
			for (String group : groups) {
				if (!group.equals(sellgroup)) {
					permission.playerRemoveGroup((String) null, group, player);
				}
			}
			permission.playerAddGroup((String) null, sellgroup, player);
			plugin.Debug(player+" set to group "+sellgroup);
			*/
			plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "permissions player setgroup " + player + " " + sellgroup);
			return sellgroup;
		} else {	
			String rentgroup = plugin.configHandler.getRentgroups().get(count+1);
			if (rentgroup != null) {
				plugin.Debug(player+" added to group "+rentgroup);
				permission.playerAddGroup((String) null, rentgroup, player);
			} else {
