...
public class EditCommands extends CommandHelper {
	...
	public void execute(CommandSender sender, String Command, List<String> list) {
		...
		if (Command.equals("settype")) {
			...
			if (type == null) {
				this.error("Unbekannter Typ: " + list.get(0));
				return;
			}
			ms.setType(type);
			plugin.marketHandler.update(ms); // TODO: TEST FOR RENTED / RENT
												// (Owner problems?)
			this.reply("Der Typ wurde erfolgreich ge�ndert!");
		}

		if (Command.equals("setregion")) {
			ProtectedRegion region = plugin.regionHandler.getRegion(player.getWorld(), list.get(0));
			if (region == null) {
				this.error("Es konnte keine Region unter der ID " + list.get(0) + " gefunden werden.");
			} else {
				ms.setRegion(region);
				plugin.marketHandler.update(ms);
				this.reply("Die Region wurde erfolgreich geändert!");
			}
			return;
		}
		...
		if (Command.equals("setprice")) {
			if (!list.get(0).matches("\\d*")) {
				...
				if ((gp != null) && player.hasPermission("XcraftRegionMarket.GP.Use")) {
					...
					this.error("Der Preis (" + list.get(0) + ") darf nur Zahlen enthalten.");
					return;
				}
			} else {
				ms.setPrice(Double.parseDouble(list.get(0)));
			}
			plugin.marketHandler.update(ms);
			this.reply("Der Preis wurde erfolgreich ge�ndert!");
			return;
		}
		if (Command.equals("setowner")) {
			...
			plugin.marketHandler.update(ms);
			this.reply("Der Besitzer wurde erfolgreich ge�ndert!");
			return;
		}

		if (Command.equals("setintervall")) {
			if ((ms.getType() == Type.SELLING) || (ms.getType() != Type.SOLD)) {
				this.error("Dieses Schild muss vom Typ \"rent\" oder \"rented\" sein!");
				return;
			}
			...
			plugin.marketHandler.update(ms);
			this.reply("Das Intervall wurde erfolgreich ge�ndert!");
			return;
		}
		...
		if (Command.equals("setRenter")) {
			...
			this.reply("Der Vermieter wurde erfolgreich ge�ndert!");
			return;
		}
	}
}
